...
import { DARK, LIGHT, UI_THEME_KEYS } from "../redux/utils";

export const useTheme = () => {
  const [theme, setTheme] = useState<PaletteMode>(LIGHT);
  const [labelTheme, setLabelTheme] = useState<string>(
    UI_THEME_KEYS.LABEL_LIGHT,
  );
  ...
  const toggleTheme = (isDark: boolean) => {
    ...
    setLabelTheme(
      isDark ? UI_THEME_KEYS.LABEL_DARK : UI_THEME_KEYS.LABEL_LIGHT,
    );
  };
